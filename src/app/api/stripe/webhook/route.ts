...
async function maybeLoadPreview(previewId: string): Promise<PreviewData | null> {
  ...
  try {
    ...
    console.warn("[webhook] preview load failed:", (e as Error).message);
    return null;
  }
}
  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      console.log("[webhook] parsed", {
        email, previewId, planKey, amount: session.amount_total, currency: session.currency
      });

      // 1) Upsert order (best-effort)
      try {
        const id = uuid4();
        const previewUUID =
      ...
      // 2) Build data (from preview if available, otherwise empty defaults)
      ...
      // 4) Email report (if we have an email)
      if (email) {
